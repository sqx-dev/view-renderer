...
import { checkBoxStatusRecursively, launchSandbox } from './api';
import { initilizeFirebaseOnLoad } from './dom-helpers.js';
import { togglePreventNavigationEvent } from './prevent-navigation.js';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  UI.boxType = 'Disposable File Viewer';

  document.querySelector(
    '.heading.blue.loading-head'
  ).innerHTML = `Preparing ${UI.boxType}`;

  UI.session = urlSession ?? localStorage.getItem('boxSession');
  UI.terminationToken =
    localStorage.getItem('boxSessionTerminationToken') ?? UI.session;
  UI.sessionInURL = urlSession;
});
...
function toggleInfoBox(state) {
  const backDrop = document.querySelector('.modal-back-drop');
...
window.addEventListener('load', () => {
  initilizeFirebaseOnLoad(UI);

  UI.initiateConnection(true);
...
const UI = new Proxy(
  {
    ...
    addPropertyListener(property, listener) {
      if (property === 'listeners') {
        console.warn('Cannot listen to listeners just yet...');
        return;
      }

      const listeners = UI.listeners[property] ?? [];
      listeners.push(listener);
      UI.listeners[property] = listeners;
    },
    ...
    removePropertyListener(property, listener) {
      ...
      const l_idx = listeners.indexOf(listener);

      if (l_idx === -1) {
        return;
      }

      listeners.splice(l_idx, 1);
    ...
    async initiateConnection(isInitialCall) {
      UI.forceSetting('resize', 'remote');
      document.body.classList.remove('no-controls');

      if (UI.sessionInURL) {
        UI.session = UI.sessionInURL;

        // UI.connect();
        this.checkBoxStatus(isInitialCall);
      } else {
        UI.session = localStorage.getItem('boxSession');

        if (UI.session) {
          return this.checkBoxStatus(isInitialCall);
        }

        const launchResponse = await launchSandbox(UI);

        if (launchResponse.success && launchResponse.result) {
          UI.session = launchResponse.result.session;
          UI.terminationToken = launchResponse.result.termination_token;
          this.checkBoxStatus(isInitialCall);

          localStorage.setItem('boxSession', UI.session);
          localStorage.setItem(
            'boxSessionTerminationToken',
            UI.terminationToken
          );
        } else {
    ...
    async checkBoxStatus(isInitialCall) {
      if (!this.session) {
        return;
      }

      if (UI.sessionDisposed) {
        UI.shouldRetryConnection = false;
        return;
      }

      const boxStatusApiResponse = await checkBoxStatusRecursively(
        this.session,
        UI,
        isInitialCall ? Infinity : 10
      );

      if (boxStatusApiResponse.success) {
        UI.shouldRetryConnection = true;
        audio.initializeAudioAndConnectToWs(UI);
        ...
        UI.sessionDisposed =
          boxStatusApiResponse.message === 'Session Disposed';
        UI.shouldRetryConnection = false;
        UI.handleErrors(boxStatusApiResponse.message);
      }
    },

    handleErrors(message) {
      document.body.classList.add('no-controls');
      document.documentElement.classList.add('lost');

      UI.showErrorOverlay(message);
    ...
    addKeyboardControlsPanelHandlers() {
      ...
      interact('.keyboard-controls .button.windows').on(
        'tap',
        UI.toggleWindows
      );
    ...
    addConnectionControlHandlers() {
      ...
      let connectBtnEl = document.getElementById('noVNC_connect_button');
      if (typeof connectBtnEl != 'undefined' && connectBtnEl != null) {
        connectBtnEl.addEventListener('click', UI.connect);
      }
      document
    ...
    updateVisualState(state, message) {
      document.documentElement.classList.remove('noVNC_error');
      ...
      const transitionElem = document.querySelector('.loading-head');
      if (WebUtil.isInsideKasmVDI()) {
      ...
      switch (state) {
        ...
        case 'connecting':
          toggleInfoBox(false);
          transitionElem.textContent = _(message || `Loading ${UI.boxType}...`);
        ...
        case 'disconnecting':
          toggleInfoBox(false);
          transitionElem.textContent = _(`Disconnecting ${UI.boxType}...`);
          document.documentElement.classList.add('noVNC_disconnecting');
          break;

        case 'offline':
          toggleInfoBox(false);
          document.querySelector('.heading-error').textContent =
            'You are offline. Please connect to the internet';
          document.documentElement.classList.add('noVNC_error');
          break;
        ...
        case 'reconnecting':
          toggleInfoBox(false);
          transitionElem.textContent = _(`Reconnecting ${UI.boxType}...`);
        ...
        case 'error':
          toggleInfoBox(false);
          document.querySelector('.heading-error').textContent =
            message || 'SquareX Encountered An Error';
          document.documentElement.classList.add('noVNC_error');
          break;
    ...
    showStats() {
      ...
      let enableStats = UI.getSetting('enable_perf_stats');
      if (enableStats === true && UI.statsInterval == undefined) {
        document.getElementById('noVNC_connection_stats').style.visibility =
          'visible';
        UI.statsInterval = setInterval(() => {
          if (UI.rfb !== undefined) {
    ...
    addClickHandle(domElementName, funcToCall) {
      ...
      const control = document.getElementById(domElementName);
    ...
    updateSetting(name) {
      ...
      if (ctrl.type === 'checkbox') {
        ...
        let valueLabel = document.getElementById(
    ...
    clipboardReceive(e) {
      if (UI.rfb.clipboardDown) {
        const curvalue = document.getElementById('noVNC_clipboard_text').value;
    ...
    connect(event, password) {
      const requestUrl = `wss://${UI.session}`;
      ...
      if (WebUtil.isInsideKasmVDI()) {
        ...
        UI._sessionTimeoutInterval = setInterval(() => {
    ...
    cancelReconnect() {
      ...
      UI.showErrorOverlay('Connection Lost');
    ...
    connectFinished(e) {
      ...
      document.getElementById('session-info-box')?.classList.remove('hidden');
    ...
    showErrorOverlay(message) {
      ...
      if (this.isOnline === false) {
        UI.updateVisualState('offline');
      } else {
        UI.updateVisualState('error', message);
      }

      if (message !== 'Session Disposed') {
        document.documentElement.classList.remove('lost');
      }
    },

    async disconnectFinished(e) {
      togglePreventNavigationEvent(false);
      ...
      if (UI.sessionDisposed) {
        UI.handleErrors('Session Disposed');
        return;
      }
    ...
    promiseConnection() {
      return new Promise((resolve) =>
        resolve(window.setTimeout(UI.connect, 2000))
      );
    },

    async retryConnection() {
      if (UI.reconnectTries === 0) {
        UI.forceReconnect = false;
        UI.reconnectTries = 5;
        UI.showErrorOverlay('Failed To Connect To Server');
      ...
      try {
        UI.reconnectTries -= 1;
        // await UI.promiseConnection()
        window.setTimeout(UI.connect, 2000);
        // UI.reconnect();
      } catch (error) {
        UI.forceReconnect = false;
        UI.reconnectTries = 5;
      ...
      if (event.data && event.data.action) {
        switch (event.data.action) {
          ...
          case 'set_idle_timeout': {
            ...
            const idleTimeoutMin = Math.ceil(event.data.value / 60);
            UI.forceSetting('idle_disconnect', idleTimeoutMin, false);
            UI.rfb.idleDisconnect = idleTimeoutMin;
    ...
    disconnectedRx(event) {
      parent.postMessage(
        { action: 'disconnectrx', value: event.detail.reason },
        '*'
      );
    },
    ...
    toggleRelativePointer(event = null, forcedToggleValue = null) {
      ...
      const togglePosition = !UI.rfb.pointerRelative;
    ...
    updateQuality(fps) {
      let presentMode = parseInt(UI.getSetting('video_quality'));
      let enableQoi = false;
      ...
      if (UI.rfb) {
        ...
        UI.rfb.dynamicQualityMin = parseInt(
          UI.getSetting('dynamic_quality_min')
        );
        UI.rfb.dynamicQualityMax = parseInt(
          UI.getSetting('dynamic_quality_max')
        );
        UI.rfb.jpegVideoQuality = parseInt(UI.getSetting('jpeg_video_quality'));
        UI.rfb.webpVideoQuality = parseInt(UI.getSetting('webp_video_quality'));
        UI.rfb.videoArea = parseInt(UI.getSetting('video_area'));
    ...
    inputLockChanged(e) {
      const pointerLockEl = document.getElementById(
        'noVNC_setting_pointer_lock'
      );
      // const pointerRelEl = document.getElementById('noVNC_game_mode_button');

      if (e.detail.pointer) {
        pointerLockEl.checked = true;
  ...
  {
    // Allows us to listen to changes in the individual properties
    // of the UI object.
    set: (a, b, c, d) => {
      const x = Reflect.get(a, b, d);
      const t = Reflect.set(a, b, c, d);
      if (c !== x) {
        (UI.listeners[b] ?? []).forEach((v) => v(c, UI));
      }
      return t;
    },
  }
